...
// Implementation methods ----------------------------------------------------------------------
  ...
  private static NumericRangeQuery<? extends Number> 
    ...
    if (min instanceof Double || (min == null && max instanceof Double)) {
      return NumericRangeQuery.newDoubleRange(field, (Double)min, (Double)max, true, true);
    }
    // Float
    if (min instanceof Float || (min == null && max instanceof Float)) {
      return NumericRangeQuery.newFloatRange(field, (Float)min, (Float)max, true, true);
    }
    // Should never happen
    return null;
  }
