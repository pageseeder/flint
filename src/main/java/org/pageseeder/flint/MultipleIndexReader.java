...
public final class MultipleIndexReader {
  ...
  protected MultipleIndexReader(IndexManager manager, List<Index> indexes) {
  ...
  public IndexReader grab() throws IndexException {
    ...
    for (Index index : this._indexes.keySet()) {
      // grab what we need
      IndexReader reader = this._manager.grabReader(index);
      readers[i++] = reader;
  ...
  public void releaseSilently() {
    ...
    while (!this._indexes.isEmpty())  {
      ...
      this._manager.releaseQuietly(index, reader);
    }
  }
}